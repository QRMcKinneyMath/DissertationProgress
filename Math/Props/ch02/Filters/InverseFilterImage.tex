...
\begin{enumerate}
\item $X$ and $Y$ be nonempty sets.
\item $f:X \to Y$. 
\item $\scB$ is a \FilterBase for a \Filter $\scF$ on $Y$. 
\item $\scF_{f(X)}=\{U \cap f(X) : U \in \scF\}$
\item $\scG$ is a \Filter on $Y$. 
\item $\scG_{f(X)}=\{U \cap f(X) : U \in \scG\}$
\end{enumerate}
...
\begin{enumerate}[label=(\roman*), ref={\ref{prop:InverseFilterImage}.~\roman*}]
    ...
    then $f\pa{f^{-1}\pa{\scB}}$ is a \FilterBase for a \Filter on $Y$ 
    \FinerFilter
    than $\scF$. 
    \item \label{prop:InverseFilterImage:Subspace} 
    If $f\pa{f^{-1}\pa{\scB}}$ is a \FilterBase 
    for 
    $\scG$, then $\scG_{f(X)}=\scF_{f(X)}$. 
\end{enumerate}
...
\begin{proof}[Proof of \ref{prop:InverseFilterImage:Subspace}]
    If $f \pa{f^{-1}\pa{\scB}}$ is a \FilterBase for $\scG$, then 
    $\emptyset \not \in f^{-1}\pa{\scB}$. 
    By \ref{prop:InverseFilterImage:Base}, 
    we then conclude $f^{-1}\pa{\scB}$ is a \FilterBase on $X$. 
    That allows us to leverage 
    \ref{prop:InverseFilterImage:Finer}
    to conclude that $\scF \leq \scG$.
    Furthermore, 
    \begin{equation*}
    f\pa{f^{-1}\pa{\scB}} = \{f(X) \cap B : B \in \scB\} \subset scF_{f(X)}
    \end{equation*}
    so that $\scG_{f(X)} \leq  \scF_{f(X)}$.
    Since the inclusion goes both ways, equality holds.
\end{proof}
\end{prop}
