...
\begin{enumerate}
    ...
    \item For each $\gamma \in A$, $\pi_\gamma : \prod\limits_{\alpha \in A} X_\alpha \to  X_\gamma$ represents the $\gamma-$\ProjectionMap. 
...
\begin{proof}[Proof of \ref{prop:ProductFilterFacts:Existence}]
    ...
    For $i,j \in \{1,\cdots,n\}$, define $i \equiv j$ if and only if $\alpha_i = \alpha_j$.
    ...
    $\{V_p\}_{p \in k} \subset \scF_{\alpha_{min(k)}}$, 
    and also
...
\begin{proof}[Proof of \ref{prop:ProductFilterFacts:Subbasis}]
    ...
    By 
    \ref{prop:FilterBaseFacts:BaseFromSubbasis}, 
    for each 
    $\alpha \in A$, $\scK_\alpha$ is a \FilterBase for $\scF_\alpha$. 
    An application of 
    \ref{prop:ProductFilterFacts:Base} implies 
