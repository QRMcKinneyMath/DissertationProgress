...
\begin{enumerate}[label=(\roman*), ref={\ref{prop:BLO}~\roman*}]
\item 
\label{prop:BLO:WellDefined}
$\norm{\cdot}$ is a well-defined \Seminorm on $BL(X,Y)$. 
\item 
\label{prop:BLO:Norm}
If $\norm{\cdot}_Y$ is a \Norm, then so is $\norm{\cdot}$. 
\item 
\label{prop:BLO:Sphere}
If $T \in BL(X,Y)$ and $\alpha \in (0,\infty)$, then $\norm{T} = \sup\limits_{\norm{x}_X =\alpha} \frac{\norm{Tx}_Y}{\norm{x}_X}$. 
\item 
\label{prop:BLO:Ball}
If $T \in BL(X,Y)$ and $\alpha \in (0,\infty)$, , then $\norm{T} = \sup\limits_{0<\norm{x}_X \leq \alpha} \frac{\norm{Tx}_Y}{\norm{x}_X}= \sup\limits_{0<\norm{x}_X < \alpha} \frac{\norm{Tx}_Y}{\norm{x}_X}$. 
\item 
\label{prop:BLO:Schwarz}
If $T \in BL(X,Y)$ and $x \in X$, then $\norm{Tx}_Y \leq \norm{T} \norm{x}_X$. 
\item 
\label{prop:BLO:Characterization}
$S:X \to Y$ is linear
, $S(\Ker_X) \subset \Ker_Y$
, and $\sup\limits_{\norm{x}_X \neq 0} \frac{\norm{Sx}_Y}{\norm{x}_X} < \infty$
, if and only if $S \in BL(X,Y)$. 
\item 
\label{prop:BLO:UniformlyCauchy}
A \Sequence $\{T_i\}_{i \in \N} \subset BL(X,Y)$ is a \PseudometricCauchySequence
    if and only if
    ...
    is \UniformlyCauchy.
\item 
\label{prop:BLO:NormConvergenceImpliesStrongConvergence}
If $T_i \to T$ with respect to $\norm{\cdot}$, then $T_ix \to Tx$ with respect to $\norm{\cdot}_Y$ for each $x \in X$
\item 
\label{prop:BLO:ConvergenceCriteria}
A sequence $\{T_i\}_{i \in \N} \subset BL(X,Y)$ 
...
\item 
\label{prop:BLO:Complete}
Let X be \NonDegenerate. Then $BL(X,Y)$ is complete if Y is complete. 
\item  
\label{prop:BLO:NonDegenerate}
If $BL(X,Y)$ is \NonDegenerate then Y is \NonDegenerate.
\item 
\label{prop:BLO:Bounded}
If $S:X \to Y$ is \Linear,
	...
	$\norm{Tx} \leq c \norm{x}$. 
\item 
\label{prop:BLO:NormCharacterization}
If $T \in BL(X,Y)$, 
...
\begin{proof}[Proof of \ref{prop:BLO:WellDefined}] 
...
\end{proof}
\begin{proof}[Proof of \ref{prop:BLO:Norm}] 
   ...
   \end{equation}
\end{proof}
\begin{proof}[Proof of \ref{prop:BLO:Sphere}] 
...
\end{proof}
\begin{proof}[Proof of \ref{prop:BLO:Ball}] 
If we define, for 
$T \in BL(X,Y)$, 
...
\end{proof}
\begin{proof}[Proof of \ref{prop:BLO:Schwarz}]
...
so that $\norm{Tx}_Y \leq \norm{T} \norm{x}_X$. 
\end{proof}
\begin{proof}[Proof of \ref{prop:BLO:Characterization}]
	...
	\Linear 
	by definition
    , and the supremum expression is finite by \ref{prop:BLO:WellDefined}
    of this result. 
    Since S is 
	\Linear, $S0_X = 0_Y$. 
    Since S is 
...
\end{proof}
\begin{proof}[Proof of \ref{prop:BLO:UniformlyCauchy}]
    ...
    and applying \ref{prop:BLO:Sphere}, 
    $\norm{T_m-T_n} < \epsilon$. 
\end{proof}
\begin{proof}[Proof of \ref{prop:BLO:NormConvergenceImpliesStrongConvergence}]
    Let $T_i \to T$. 
    Let $x \in X$. 
    ...
    \end{equation*}
\end{proof}
\begin{proof}[Proof of \ref{prop:BLO:ConvergenceCriteria}]
    
    $(\implies)$ 
    Suppose $T_i \to T$.
    Then, by \ref{prop:pseudometricconvergenceimpliespseudometriccauchy}, 
    $\{T_i\}_{i \in \N}$ is a \PseudometricCauchySequence.
    An application of \ref{prop:BLO:NormConvergenceImpliesStrongConvergence} implies
    ...
    and since \ref{prop:BLO:UniformlyCauchy}, 
    ...
    \frac{\norm{(T_n-T)x}_Y}{\norm{x}_X} < \epsilon
    \end{equation}
    Implying first by \ref{prop:BLO:Characterization}
    that $T \in BL(X,Y)$, and second that
    Hence $T_i \to T$ with respect to $\norm{\cdot}$. 
\end{proof}
\begin{proof}[Proof of \ref{prop:BLO:Complete}]
    ...
    This allows us apply \ref{prop:BLO:ConvergenceCriteria}
...
\begin{proof}[Proof of 11]
    ...
    $\norm{Tx}_Y \neq 0$. 
\end{proof}
\begin{proof}[Proof of \ref{prop:BLO:Bounded}]
	Let $S \in BL(X,Y)$. 
	...
	\end{equation}
	so multiplying by $\norm{x}$ gives $\norm{Sx} \leq c \norm{x}$. 
	If $\norm{x} = 0$, then $x \in \Ker_X$.
    Hence, by \ref{prop:BLO:Characterization}, $Sx \in \Ker_X$, so 
	\begin{equation}
		\norm{Sx} =0 = c 0  = c \norm{Sx}
	\end{equation}
...
\end{proof}
\begin{proof}[Proof of \ref{prop:BLO:NormCharacterization}]
	By \ref{prop:BLO:Schwarz}, $\norm{T}$ is one possible value
	for c, so $\norm{T} \in A$. 
	That is, $inf(A) \leq \norm{T}$. 
...
\rm
The converses of 
\ref{prop:BLO:Complete}
and
\ref{prop:BLO:NonDegenerate}
are true, but their proof relies upon
the Hahn Banach theorem for \SeminormedSpaces, 
