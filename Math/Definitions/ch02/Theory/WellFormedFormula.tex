...
\rm
    ...
    \begin{enumerate}[label=(\roman*), ref={\ref{def:WellFormedFormula}.~\roman*}]
        ...
        $C \StringEqual ((\forall y)B)$ where 
        $B$ is a \WFF in \scG and 
        $y$ is an \IndividualVariable
        such that the string $(\forall y)$ is not a \Substring of $B$. 
    \end{enumerate}
    the collection of \Wffs of \scG is the collection of 
    \Statements of \scG. 
\end{df}
